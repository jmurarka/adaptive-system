"""
Adaptive Learning System — FastAPI Backend (MongoDB + Neo4j Integrated)
"""
...
# ── Runtime State (still needed for fast access) ──────
...
def create_user_api(req: CreateUserRequest):
    ...
    create_learner_state(user["user_id"])
    create_user_node(user["user_id"])
    sync_user_to_neo4j(user["user_id"])
...
def update_mastery(req: UpdateMasteryRequest):
    ...
    # 🔥 Neo4j sync
    sync_user_to_neo4j(req.learner_id)

    # 🔥 Dynamic KG trigger
    if state.level.value == "strong":
        handle_mastery_update(req.learner_id, req.concept_id)
...
def get_plan(learner_id: str):
    ...
    pos = _canonical_positions.get(learner_id, 0)
    return replanner.generate_roadmap(learner, pos)
...
def get_analysis(learner_id: str):
    learner = get_learner(learner_id)
    pos = _canonical_positions.get(learner_id, 0)
    return reasoner.analyse(learner, pos).to_dict()
